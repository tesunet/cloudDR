...
class CV_Backupset(CV_Client):
    ...
    def _createMSSqlInstance(self, clientName, instanceName, platform, credit):
        ...
        output = "script/sqlCreat-" + credit["Server"] + ".xml"
    ...
    def _modiMSSqlInstance(self, clientName, instanceName, platform, credit):
        input = "template/sqlModify.xml"
        input1 = "template/oraCreate.xml"
        output = "script/sqlModi-" + credit["Server"] + ".xml"
        instanceName = credit["instanceName"]
        SPName = credit["SPName"]
        useVss = credit["useVss"]
        ...
        for node in nodes:
            if node.text == '0':
                self.msg = "modify mssql instance success"
            else:
                errnodes = root.findall(".//errorString")
                for errnode in errnodes:
                    self.msg = errnode.text
    ...
    def restoreMssqlBackupset(self, source, dest, operator):
        ...
        output = "script/sqlRestore-" + dest + ".xml"
